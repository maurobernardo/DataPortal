-- Migração de produção — actualizado em 2026-08-24, cumulativo (inclui tudo desde o início).
-- Corre isto no phpMyAdmin da base de dados de produção, uma vez, depois de publicar o novo código.
-- Se der erro "Duplicate column name" ou "Table already exists", é normal: já correu antes, ou a
-- própria app cria/actualiza estas tabelas sozinha na primeira consulta (todo o `lib/db.ts` e
-- `lib/analysis/*.ts` fazem isto de forma idempotente). O phpMyAdmin continua para a instrução
-- seguinte mesmo que uma falhe — não precisas de remover as linhas que já correram.

-- ============================================================================
-- JÁ APLICADO EM MIGRAÇÕES ANTERIORES (mantido aqui só para o script ficar completo)
-- ============================================================================

-- 1) Activar/desactivar contas sem apagar dados (gestão de utilizadores no admin)
ALTER TABLE users ADD COLUMN active TINYINT(1) NOT NULL DEFAULT 1;

-- 2) Log de auditoria (quem promoveu/removeu admin, activou/desactivou conta, criou/eliminou dataset)
...
-- 3) Filtro por sector nos relatórios (/relatorios)
ALTER TABLE Report ADD COLUMN sector VARCHAR(80) NULL;

-- ============================================================================
-- NOVO NESTA LEVA — 2FA, eliminação de conta com prazo, pedido de eliminação
-- ============================================================================
...
ALTER TABLE users ADD COLUMN oauth_id VARCHAR(191) NULL;
ALTER TABLE users MODIFY COLUMN password_hash VARCHAR(255) NULL;
ALTER TABLE users ADD UNIQUE INDEX users_oauth_provider_id_key (oauth_provider, oauth_id);
ALTER TABLE users ADD COLUMN pending_email VARCHAR(254) NULL;
ALTER TABLE users ADD COLUMN pending_email_code VARCHAR(6) NULL;
ALTER TABLE users ADD COLUMN pending_email_expires DATETIME(3) NULL;
...
ALTER TABLE users ADD COLUMN profile_category VARCHAR(30) NULL;
ALTER TABLE users ADD COLUMN pedido_eliminacao_em DATETIME(3) NULL;

-- ============================================================================
-- Categorias: mesmo nome permitido em tipos diferentes (ex.: "Agricultura" geo + alfanumérico)
-- ============================================================================

-- Só corre se ainda existir o índice antigo "só por nome"; se der erro "check that column/key
-- exists" é porque já foi migrado.
...
-- ============================================================================
-- Dataset: badge/miniatura, certificação de proveniência, resumo automático por IA
-- ============================================================================

ALTER TABLE Dataset ADD COLUMN previewAvailable TINYINT(1) NULL;
ALTER TABLE Dataset ADD COLUMN bboxMinX DOUBLE NULL;
ALTER TABLE Dataset ADD COLUMN bboxMinY DOUBLE NULL;
ALTER TABLE Dataset ADD COLUMN bboxMaxX DOUBLE NULL;
ALTER TABLE Dataset ADD COLUMN bboxMaxY DOUBLE NULL;
ALTER TABLE Dataset ADD COLUMN certificacao VARCHAR(30) NOT NULL DEFAULT 'nao_verificado';
ALTER TABLE Dataset ADD COLUMN resumoIA TEXT NULL;
ALTER TABLE Dataset ADD COLUMN resumoIAGeradoEm DATETIME(3) NULL;

-- Histórico de versões de dataset (uma linha por edição anterior, com restauro num clique)
...
-- Lixeira de datasets eliminados (recuperáveis, em vez de apagar de imediato)
...
-- Estatística diária agregada do portal (para os alertas de limiar de uso)
...
-- Estatísticas por utilizador autenticado (associa view/download a quem estava logado)
ALTER TABLE Statistic ADD COLUMN userId INT NULL;
...
ALTER TABLE ReportRequest ADD COLUMN name VARCHAR(120) NULL;
ALTER TABLE ReportRequest ADD COLUMN email VARCHAR(254) NULL;
ALTER TABLE ReportRequest ADD COLUMN message TEXT NULL;
...
-- Mensagens de contacto: motivo do contacto
ALTER TABLE ContactMessage ADD COLUMN purpose VARCHAR(60) NULL;

-- Dashboards alfanuméricos: miniatura, categoria, contagem de vistas, última actualização dos dados
ALTER TABLE AlphanumericDashboard ADD COLUMN previewImagePath TEXT NULL;
ALTER TABLE AlphanumericDashboard ADD COLUMN category VARCHAR(191) NULL;
ALTER TABLE AlphanumericDashboard ADD COLUMN views INT NOT NULL DEFAULT 0;
ALTER TABLE AlphanumericDashboard ADD COLUMN lastDataUpdate DATE NULL;
...
-- Motor de análise: detalhe de confiança guardado por análise
ALTER TABLE analises ADD COLUMN confianca_detalhe LONGTEXT NULL;

-- Rótulos aprendidos (nomes amigáveis para colunas/valores, ensinados pelo uso)
...
-- Perfil calculado de cada dataset (estatísticas, correlações) em cache
...
-- Registo de falhas do motor de análise (para diagnóstico)
...
-- ============================================================================
-- Sugestões de Datasets (admin) — Fases 1-5: classificação, estado, tipos por categoria
-- ============================================================================
