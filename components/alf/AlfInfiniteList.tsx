...
interface AlfInfiniteListProps {
...
export function AlfInfiniteList({
  ...
  const loadDatasetsFromAPI = async (offset: number, take: number) => {
    try {
      ...
      queryParams.append('dataType', 'alfanumerico')
...
export function AlfEmptyState({ hasFilters }: { hasFilters?: boolean }) {
  return (
    <div className="geo-empty-state">
      <Database className="w-10 h-10 mx-auto mb-4 text-[var(--pd-ink-300)]" />
      <h3 className="text-lg font-semibold text-[var(--pd-ink-900)] mb-2">
        {hasFilters ? 'Nenhum dataset encontrado' : 'Dados alfanuméricos em breve'}
      </h3>
      <p className="text-sm text-[var(--pd-ink-500)] mb-6 max-w-md mx-auto">
        {hasFilters
          ? 'Não encontramos datasets que correspondam aos seus critérios. Ajuste os filtros ou limpe a pesquisa.'
          : 'Estamos a preparar a publicação de novos datasets tabulares. Volte em breve ou explore os dados geoespaciais.'}
      </p>
      <a
        href={hasFilters ? '/dados-alfanumericos' : '/dados-espaciais'}
        className="geo-ldp-action-primary inline-flex"
      >
        {hasFilters ? 'Limpar filtros' : 'Explorar geoespacial'}
      </a>
    </div>
  )
}
