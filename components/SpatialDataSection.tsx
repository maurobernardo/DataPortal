  ...
  const features = [
    {
      ...
      href: '/dados-espaciais?sortOrder=newest',
    },
    {
      icon: Globe,
      ...
      href: '/dados-alfanumericos?sortOrder=newest',
    },
  ]
  const quickFilters = [
    { label: 'Mais populares', href: '/dados-espaciais' },
    { label: 'Mais recentes', href: '/dados-espaciais?sortOrder=newest' },
    { label: 'Shapefile', href: '/dados-espaciais?format=Shapefile' },
    { label: 'GeoJSON', href: '/dados-espaciais?format=GeoJSON' },
  ]
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10 lg:gap-12 items-center">
          ...
          <div className="space-y-8 animate-slide-up">
              ...
              {features.map((feature, index) => {
                ...
                return (
                  <RevealOnScroll key={index} delayMs={index * 100}>
                    <Link href={feature.href} className="home-card flex items-center gap-3 px-6 py-4">
            ...
            <div className="flex flex-wrap gap-3">
              {quickFilters.map((filter) => (
                <Link
                  key={filter.label}
                  href={filter.href}
                  className="inline-flex items-center rounded-full border border-green-200 bg-white px-4 py-2 text-sm font-semibold text-green-700 hover:bg-green-50 transition"
                >
                  {filter.label}
                </Link>
              ))}
            ...
            <div className="pt-4 animate-fade-in" style={{ animationDelay: '0.6s' }}>
              ...
              className="inline-flex items-center gap-2 md:gap-3 px-6 md:px-8 py-3 md:py-4 bg-gradient-to-r from-green-500 via-green-600 to-green-700 text-white rounded-lg md:rounded-xl font-bold text-sm md:text-base lg:text-lg shadow-2xl hover:shadow-green-500/50 hover:scale-105 transition-all duration-300 group"
              >
